...
def generate_json():
    ...
    <h2>Examples of Use</h2><ul>{''.join([
        f'<li><a href="{ex["url"]}" target="_blank">{ex["title"]}</a> - {ex["institution"]}</li>' for ex in         data.get("examples", [])
    ])}</ul>
    <h2>Key Information</h2><ul>{''.join([
        f'<li><strong>{info["label"]}:</strong> <a href="{info["url"]}" target="_blank">{info["value"]}</a></li>'
        if "url" in info else f'<li><strong>{info["label"]}:</strong> {info["value"]}</li>'
        for info in data.get("keyInfo", [])
    ])}</ul>
""",
